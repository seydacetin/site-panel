  ...
  <v-card flat class="mt-5">
    <v-form>
      ...
      <div class="pa-3">
        <!-- action buttons -->
        <v-card-text>
          <v-btn color="primary" class="me-3 mt-3" @click="updateUser"> DEĞİŞTİR </v-btn>
        </v-card-text>
      </div>
    </v-form>
  </v-card>
  ...
  methods: {
    ...
    async getUser() {
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:5001/showUser/${id}`)
    ...
    async updateUser() {
      if (this.newPassword !== this.cPassword) {
        ...
        try {
          await axios.put(`http://localhost:5001/updateUser/${this.$route.params.id}`, {
